...
String helpKey = "ExpN10_1"; //$NON-NLS-1$

String breite = "100%"; //$NON-NLS-1$
String rechteFehler = appBean.getErrorIfRightsAreMissing(JspPage.ADM_N10_1_EXPORT); 
...
DialogStateHolder p4ES = expP4Bean.getP4ExportStateN10_1();
WahlInfo wahlInfo = WahlInfo.getWahlInfo();

String i18nName = "Export_P4_N10_1"; //$NON-NLS-1$
String i18nTitle = "Export_P4_titel_N10_1"; //$NON-NLS-1$
                               ...
                               <tr>
                                  <td valign="top">
                                     ...
                                     if (exportVerz == null || exportVerz.isEmpty()){ %>
                                        ...
                                        <% } else {
                                    if (ExportWithoutParametersState.STATUS_P4_D1 == p4ES._modus){
                                            String urlExp = "/osv?cmd=" + Action.EXP_P4_EXPORT_P4_N10_1.getKey() + "&" + ClientHelper.getAllParameters(request); //$NON-NLS-1$ $NON-NLS-2$
                                                   %>
                                              <table border="0" cellspacing="0" cellpadding="1" width="<%= breite %>">
                                                 <tr>
                                                    <td valign="top">
                                                               ...
                                                               <tr>
                                                                    <%  String propCentralCounting = expP4Bean.getProperty(Konstanten.PROP_EXPORTS_FOR_CENTRAL_COUNTING);
                                                                        if (propCentralCounting == null) {
                                                                            propCentralCounting = Konstanten.OPTION_NO_CENTRAL_COUNTING;
                                                                        }
                                                                    %>
                                                                       <td><ivu:int key="Export_for_central_counting_N10_1"/></td>
                                                                       <td><select name="<%=ApplicationBeanKonstanten.PREFIX + Konstanten.PROP_EXPORTS_FOR_CENTRAL_COUNTING%>">
                                                                                <option value="<%=Konstanten.OPTION_NO_CENTRAL_COUNTING%>"<%= propCentralCounting.equals(Konstanten.OPTION_NO_CENTRAL_COUNTING) ? " selected=\"selected\"" : "" %>>Model N10</option>
                                                                                <option value="<%=Konstanten.OPTION_CENTRAL_COUNTING%>"<%= propCentralCounting.equals(Konstanten.OPTION_CENTRAL_COUNTING) ? " selected=\"selected\"" : "" %>>Model I en lege bijlage 2 van Model II</option>
                                                                            </select>
                                                                        </td>
                                                                  </tr>
                                                                <tr>
                                     ...
                                     <% } else if (ExportWithoutParametersState.STATUS_EXPORT_FINISHED == p4ES._modus){
                                                    // reset export status
                                                    expP4Bean.resetExportStateN10_1();
                                                    //forward to Werkmap
                                                    String urlExp = "/osv?cmd=" + Action.EXP_P4_EXPORT_P4_N10_1.getKey() + "&" + ClientHelper.workIs(Command.EXPORT_VERZEICHNIS) + "&" + ClientHelper.getAllParameters(request, ApplicationBeanKonstanten.WORK); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                                        %>
                                        <fieldset style="border: 1px solid #093C69; padding: 15px">
                                                        <legend><b><ivu:int key="<%=i18nName%>"/></b></legend>
