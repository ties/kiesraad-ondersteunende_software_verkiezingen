...
/**
 * Original code by Yashwant Chavan, modified by J. Nottebaum and T. Ducke
 * 
...
public class FolderZiper {
  ...
  public static void zipFolderFilterResults(String electionId,
      String destZipFile,
      String... srcFolders) throws Exception {
  ...
  private static void addFolderToZip(String path,
    ...
    if (folder.isDirectory()) { // only add existing folders
      ...
      for (String fileName : folder.list()) {
        addFileOrFolderToZip(newName,
            srcFolder + SEPARATOR + fileName,
            zip,
            filterResults,
            electionId);
      }
  ...
  private static boolean isResultFile(String srcFile, String electionId) {
    ...
    if (filename.endsWith(".eml.xml") //$NON-NLS-1$
        && (filename.startsWith(RgConstants.FILENAME_110A)
            ...
            || filename.startsWith(RgConstants.FILENAME_510_COMPLETE)
            && !filename.contains(RgConstants.FILENAME_FRAGMENT_STEMBUREAU)
            || filename.startsWith(RgConstants.FILENAME_510D_COMPLETE) || filename
              .startsWith(RgConstants.FILENAME_520)) && isContainsElectionId(filename, electionId)) {
      return true;
    }
    return false;
  }
  ...
  private static boolean isContainsElectionId(String srcFile, String electionId) {
    ...
    /*
     * Split election domain from category and year part, because Totaaltelling filename has the
     * word gemeente in between.
     */
