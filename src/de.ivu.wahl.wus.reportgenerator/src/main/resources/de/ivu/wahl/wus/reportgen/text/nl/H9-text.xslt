...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:param name="H9_110_naam" select="'naam'"/>
    <xsl:param name="H9_120_voorletters" select="'voorletters'"/>
    <xsl:param name="H9_130_postadres" select="'postadres'"/>
    <xsl:param name="H9_140_postcode" select="'postcode'"/>
    <xsl:param name="H9_150_plaats" select="'plaats'"/>
    <xsl:param name="H9_160_AdreskKnnisgeving" select="'Adres voor de kennisgeving van mijn benoeming'"/>
    <xsl:param name="H9_170_invullenAlsGeenGemachtigde" select="'(invullen als u geen gemachtigde aanwijst)'"/>
    <xsl:param name="H9_180_kennisgevingAddress" select="'Ik wil de kennisgeving van mijn benoeming ontvangen op:'"/>
    <xsl:param name="H9_190_Postadres" select="'Postadres'"/>
    <xsl:param name="H9_200_Postcode" select="'Postcode'"/>
    <xsl:param name="H9_210_Plaats" select="'Plaats'"/>
    <xsl:param name="H9_215_Ondertekening" select="'Ondertekening door de kandidaat'"/>
    <xsl:param name="H9_220_Datum" select="'Datum'"/>
    <xsl:param name="H9_230_Naam" select="'Naam'"/>
    <xsl:param name="H9_240_Woonplaats" select="'Woonplaats'"/>
    <xsl:param name="H9_250_Burgerservicenummer" select="'Burgerservicenummer'"/>
    <xsl:param name="H9_260_Handtekening" select="'Handtekening'"/>
</xsl:stylesheet>
