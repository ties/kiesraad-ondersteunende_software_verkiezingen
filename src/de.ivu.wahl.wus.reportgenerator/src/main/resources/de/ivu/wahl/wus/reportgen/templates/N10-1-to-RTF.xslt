...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:altova="http://www.altova.com" xmlns:altova-xfi="http://www.altova.com/xslt-extensions/xbrl" xmlns:altovaext="http://www.altova.com/xslt-extensions" xmlns:array="http://www.w3.org/2005/xpath-functions/array" xmlns:clitype="clitype" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:eml="urn:oasis:names:tc:evs:schema:eml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:iso4217="http://www.xbrl.org/2003/iso4217" xmlns:ix="http://www.xbrl.org/2008/inlineXBRL" xmlns:java="java" xmlns:kr="http://www.kiesraad.nl/extensions" xmlns:link="http://www.xbrl.org/2003/linkbase" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:rg="http://www.kiesraad.nl/reportgenerator" xmlns:sps="http://www.altova.com/StyleVision/user-xpath-functions" xmlns:ts="urn:oasis:names:tc:evs:schema:eml:ts" xmlns:xal="urn:oasis:names:tc:ciq:xsdschema:xAL:2.0" xmlns:xbrldi="http://xbrl.org/2006/xbrldi" xmlns:xbrli="http://www.xbrl.org/2003/instance" xmlns:xff="http://www.xbrl.org/2010/function/formula" xmlns:xfi="http://www.xbrl.org/2008/function/instance" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xnl="urn:oasis:names:tc:ciq:xsdschema:xNL:2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="altova altova-xfi altovaext array clitype ds eml fn iso4217 ix java kr link map math rg sps ts xal xbrldi xbrli xff xfi xlink xnl xs xsi">
                ...
                <xsl:for-each select="eml:Count">
                    ...
                    <xsl:for-each select="eml:Election">
                        ...
                        <xsl:variable name="altova:RowStyle0">
                            ...
                            <xsl:text>\clvertalt\clpadl30\clpadfl3\clpadb30\clpadfb3\clpadt30\clpadft3\clpadr380\clpadfr3</xsl:text>
                        ...
                        <xsl:call-template name="write-text">
                            <xsl:with-param name="text">
                                <xsl:text>Met een proces-verbaal legt een stembureau verantwoording af over:</xsl:text>
                            </xsl:with-param>
                        </xsl:call-template>
                        <xsl:text>}</xsl:text>
                        <xsl:text>\par </xsl:text>
                        ...
                        <xsl:call-template name="write-text">
                            <xsl:with-param name="text">
                                <xsl:text>Het is belangrijk dat een proces-verbaal duidelijk en volledig is, want op basis van de processen-verbaal wordt de uitslag van de stemming vastgesteld. Daarnaast is het proces-verbaal openbaar. Dit betekent dat kiezers het in de dagen na de stemming mogen inzien. Alle zaken die van invloed (kunnen) zijn op het resultaat van de</xsl:text>
                            </xsl:with-param>
                        </xsl:call-template>
                        <xsl:text>}</xsl:text>
                        ...
                        <xsl:call-template name="write-text">
                            <xsl:with-param name="text">
                                <xsl:text>stemming moeten daarom in het proces-verbaal staan. Klachten of aanmerkingen van kiezers bijvoorbeeld ook. In de wet staat dit omschreven als ‘bezwaar’.  </xsl:text>
                            </xsl:with-param>
                        </xsl:call-template>
                        <xsl:text>}</xsl:text>
                        <xsl:text>\par </xsl:text>
                        ...
                        <xsl:call-template name="write-text">
                            <xsl:with-param name="text">
                                <xsl:text>proces-verbaal maken. Bovendien werkt het formulier als een checklist. Als alle onderdelen goed zijn ingevuld, dan voldoet het proces-verbaal aan de eisen van de wet. Vul het formulier daarom correct en zo volledig mogelijk in.</xsl:text>
                            </xsl:with-param>
                        </xsl:call-template>
                        <xsl:text>}</xsl:text>
                        <xsl:text>\f0\fs24</xsl:text>
                    ...
                    <xsl:choose>
                        <xsl:when test="$isElection = 0">
                            ...
                            <xsl:call-template name="write-text">
                                <xsl:with-param name="text">
                                    <xsl:text>- geldige stemmen voor de wet / het verdrag</xsl:text>
                                </xsl:with-param>
                            </xsl:call-template>
                            <xsl:text>}</xsl:text>
                            <xsl:text>\f0\fs24</xsl:text>
                    ...
                    <xsl:choose>
                        <xsl:when test="$isElection = 0">
                            ...
                            <xsl:call-template name="write-text">
                                <xsl:with-param name="text">
                                    <xsl:text>- geldige stemmen tegen de wet / het verdrag</xsl:text>
                                </xsl:with-param>
                            </xsl:call-template>
                            <xsl:text>}</xsl:text>
                            <xsl:text>\f0\fs24</xsl:text>
                    ...
                    <xsl:call-template name="write-text">
                        <xsl:with-param name="text">
                            <xsl:text>5. Verschil tussen het aantal toegelaten kiezers en het aantal getelde stembiljetten</xsl:text>
                        </xsl:with-param>
                    </xsl:call-template>
                    <xsl:text>}</xsl:text>
                    <xsl:text>\par </xsl:text>
                    ...
                    <xsl:call-template name="write-text">
                        <xsl:with-param name="text">
                            <xsl:text>Kruis een reden aan voor het verschil en geef aan hoe vaak hier sprake van was. </xsl:text>
                        </xsl:with-param>
                    </xsl:call-template>
                    <xsl:text>}</xsl:text>
                    <xsl:text>\par </xsl:text>
                    ...
                    <xsl:variable name="altova:nSumColumnWidthsTw_9" select="round(9502.00)"/>
                    ...
                    <xsl:variable name="altova:nColumnWidthTw_9_2" select="round(2.78 * 1440 * $altova:TableWidthFactor_9)"/>
                    <xsl:variable name="altova:nColumnWidthTw_9_3" select="round(2.78 * 1440 * $altova:TableWidthFactor_9)"/>
                    <xsl:text>\pard\s0\f0\fs24</xsl:text>
                    <xsl:if test="$templatetablelevel + 1 &gt; 0">\intbl</xsl:if>
                    <xsl:text>\itap</xsl:text>
                    <xsl:value-of select="$templatetablelevel + 1"/>
    ...
    <xsl:template name="altova:DocumentSection_11">
        <xsl:param name="altova:nMaxWidthIn"/>
        <xsl:param name="templatetablelevel"/>
        <xsl:text>\sect\sectd\sbkpage\facingp\pgwsxn11900\pghsxn16820\margtsxn1280\marglsxn1440\margbsxn1720\margrsxn1440\headery0\footery0\cols1\colsx720\pgndec\ftnbj\ftnrstcont\ftnnar</xsl:text>
        <xsl:text>{</xsl:text>
        <xsl:text>\headerl </xsl:text>
        <xsl:call-template name="EmptyHeader"/>
        <xsl:text>}</xsl:text>
        ...
        <xsl:for-each select="$XML">
            <xsl:for-each select="eml:EML">
                <xsl:variable name="isBesIsland" select="count(eml:Count/eml:Election/eml:ElectionIdentifier/eml:ElectionCategory[ text() = &quot;ER&quot; ]) &gt; 0 or count(eml:Count/eml:Election/eml:Contests/eml:Contest/eml:ContestIdentifier/eml:ContestName[text() = &quot;Bonaire&quot; or text() = &quot;Sint Eustatius&quot; or text() = &quot;Saba&quot;]) &gt; 0"/>
                <xsl:for-each select="eml:Count[eml:Election/eml:ElectionIdentifier/eml:ElectionCategory[text() = &apos;EP&apos; or text() = &apos;TK&apos; or text() = &apos;EK&apos; or text() = &apos;PS&apos; or text() = &apos;GR&apos; or text() = &apos;GC&apos; or text() = &apos;BC&apos; or text() = &apos;IR&apos;]]">
                    <xsl:text>\par</xsl:text>
                    <xsl:text>\pard</xsl:text>
                    <xsl:if test="$templatetablelevel + 0 &gt; 0">\intbl</xsl:if>
                    <xsl:text>\itap</xsl:text>
    ...
    <xsl:template name="altova:Section_11_FooterRight">
        ...
        <xsl:value-of select="$templatetablelevel + 1"/>
        <xsl:text>\qr\sl0\f1\fs18 </xsl:text>
        <xsl:if test="//eml:ElectionCategory[. != &quot;NR&quot; and .!= &quot;PR&quot; and . != &quot;LR&quot;]">
            <xsl:text>{\f1\fs18 </xsl:text>
            <xsl:call-template name="write-text">
                <xsl:with-param name="text">
